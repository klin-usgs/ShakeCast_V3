...
sub event_list {
    ...
    undef $SC::errstr;
    my @list;
	my $major = ($type =~ /major/i) ? ' and major_event > 0 ' : '';
	$type = 'ACTUAL' unless ($type and $type !~ /major/i);
	...
	GROUP BY
			e.event_id
		ORDER BY
			e.event_timestamp DESC, g.shakemap_version DESC
	/;
	#LIMIT $start, $count
    eval {
...
sub ago_str {
	...
	if ($time_str < 1) {
		...
		$ago = $time_str.' hour ago';
	} elsif ($time_str < 24) {
		$ago = $time_str.' hours ago';
	} elsif ($time_str < 48) {
		$ago = sprintf("%d day ago", ($time_str/24));
	} else {
		$ago = sprintf("%d days ago", ($time_str/24));
	}
...
sub newer_than {
	...
	push @args, $options->{'timestamp'};
    }
    eval {
	my $sth = SC->dbh->prepare($sql." GROUP BY e.event_id");
	my $p = SC->dbh->selectcol_arrayref($sth, undef, @args);
	$total_count = scalar @$p;
	$sth->finish;
    };
	$sql .= qq/
	GROUP BY
	...
	LIMIT ?, ?
	/;
	push @args, ($options->{'start'}, $options->{'length'});
    eval {
	my $sth = SC->dbh->prepare($sql);
	$sth->execute(@args);
	while (my $p = $sth->fetchrow_arrayref) {
...
sub from_id {
	...
	$sth = SC->dbh->prepare(qq{
	    select *, floor(time_to_sec(timediff(utc_timestamp(), event_timestamp))/3600) as ago
		FROM 
	...
	$SC::errstr = "No event for id $event_id";
    }
    return $event;
}
        ...
        my ($gridp) = SC->dbh->selectcol_arrayref(qq/
         ...
         my $sth_del_value = SC->dbh->prepare(qq/
             delete from grid_value
              where grid_id = ?/);
         my $sth_del_fac_prob = SC->dbh->prepare(qq/
             delete from facility_fragility_probability
              where grid_id = ?/);
         my $sth_del_fac_shak = SC->dbh->prepare(qq/
             delete from facility_shaking
              where grid_id = ?/);
         my $sth_del_notif = SC->dbh->prepare(qq/
             delete from notification
              where grid_id = ?/);
         my $sth_del_sta_shake = SC->dbh->prepare(qq/
             delete from station_shaking
              where grid_id = ?/);
         foreach my $grid_id (@$gridp) {
             $sth_del_value->execute($grid_id);
             $sth_del_grid->execute($grid_id);
             $sth_del_fac_prob->execute($grid_id);
             $sth_del_fac_shak->execute($grid_id);
             $sth_del_notif->execute($grid_id);
             $sth_del_sta_shake->execute($grid_id);
         }
         ...
         $sth = SC->dbh->prepare(qq/
             delete from shakemap_metric
              where shakemap_id = ?
                and shakemap_version = ?/);
        my $sth_del_shake_param = SC->dbh->prepare(qq/
             delete from shakemap_parameter
              where shakemap_id = ?
                and shakemap_version = ?/);
         foreach my $k (@$smp) {
             $sth->execute(@$k);
             $sth_del_shake_param->execute(@$k);
         }
         ...
         SC->dbh->do(qq/
          ...
          # Determine the set of shakemaps to be deleted
          ($smp) = SC->dbh->selectall_arrayref(qq/
             select SEQ
               from event
              where event_id = ?/, undef, $event_id);

        my $sth_del_fac_mod_shake = SC->dbh->prepare(qq/
             delete from facility_model_shaking
              where SEQ = ?/);
         foreach my $k (@$smp) {
             $sth_del_fac_mod_shake->execute($k);
         }
