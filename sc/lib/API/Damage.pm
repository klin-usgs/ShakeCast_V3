...
sub from_id {
	...
	my $hash_file = "$evt_dir/fac_damage.hash";
	my $hash_summary_file = "$evt_dir/fac_damage_summary.hash";
...
sub from_id_damage {
	...
	my %fac_damage;
	my $sql = qq/
	    select fs.facility_id, f.facility_name, f.facility_type, f.external_facility_id,
	...
	foreach my $metric_key (@$fac_metric) {
		...
		my $exe_sql = $class->dosubs($sql, $metric->{$metric_key});
		$sth = SC->dbh->prepare($exe_sql);
		$sth->execute($grid{grid_id}, $metric_key, @args);
	...
	my @severity_index = sort { $fac_damage{$b}->{'severity_rank'} cmp $fac_damage{$a}->{'severity_rank'}
						or $fac_damage{$a}->{'dist'} cmp $fac_damage{$b}->{'dist'}} 
		keys %fac_damage;
	$damage->{'facility_damage'} = \%fac_damage;
	$damage->{'severity_index'} = \@severity_index;
...
sub facility_evaluate_prob {
    my ($options, $facility_index) = @_;
	...
	my $sth = SC->dbh->prepare(qq/
		...
		and g.shakemap_version= ?
		AND ffm.facility_id = ?
		GROUP BY ffm.class, ffm.component
	/);
	my %fac_prob;
    #eval {
	foreach my $fac_id (@$facility_index) {
	$sth->execute($shakemap_id, $shakemap_version, $fac_id);
	while (my $p = $sth->fetchrow_hashref('NAME_lc')) {
		my %h = %$p;
