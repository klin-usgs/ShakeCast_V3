...
sub new {
  ...
  die "Parameter 'type' (interface or intraslab) in event.xml"
    unless (defined $self->{type} and 
    ($self->{type} =~ /interface|intraslab/) );
...
sub maximum {
  ...
  if ($this->{type} =~/interface/) { $Zt = 0; }
  elsif ($this->{type} =~/intraslab/) { $Zt = 1; }
  else {
