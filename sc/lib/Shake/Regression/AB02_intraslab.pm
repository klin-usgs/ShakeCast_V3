package Regression::AB02_intraslab;

# 08132007 vq Add support for finite fault
...
sub maximum {
  ...
  $M = 8.5 if ($source_type eq 'interface' and $M>8.5);
  $M = 8.0 if ($source_type eq 'intraslab' and $M>8.0);
  $H = 100 if ($H>100);

  my $delta = 0.00724 * 10**(0.507*$M);
  my $R = sqrt($Rhypo**2 + $delta**2);
  my $g = 10**(0.301-0.01*$M);
...
sub random {
  ...
  my $delta = 0.00724 * 10**(0.507*$M);
  my $R = sqrt($Rhypo**2 + $delta**2);
  my $g = ($source_type eq 'interface') ? 10**(1.2-0.18*$M) : 10**(0.301-0.01*$M);
