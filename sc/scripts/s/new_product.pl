...
# $Id: new_product.pl 64 2007-06-05 14:58:38Z klin $
...
# ShakeCast script to create a new product on the local server.  Body of
# POST request is XML defining the product.
...
eval { ($status, $message) = new_product() };
$status = SC_FAIL, $message = $@ if $@;
reply($status, $message);

# end of main

# new_product does the top level request processing.  It will either return
# a two-element list of (status, message) or it will die.
sub new_product {
    my ($xml, $sender, $product);
...
new_product.pl -- script that delivers a new ShakeCast product to a server

=head1 SYNOPSIS

 http://myserver/scripts/new_product.pl [POST]

=head1 DESCRIPTION

This script is invoked by an upstream server to deliver a new ShakeCast
product to a downstream server.  The http request is a POST; the body of the
post consists of an XML fragment describing the product.

If the receiver already knows about the product (it may have already received
the product from another server) the new product message is ignored.  Otherwise,
the new product is recorded in the database.  The product is passed on to any
