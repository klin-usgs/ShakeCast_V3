...
sub process {
    ...
    while (!$fh->eof) {
        ...
        my $component_class   = $colp->[$columns{COMPONENT_CLASS}];
        my $component   = $colp->[$columns{COMPONENT}];
        ...
        if (%probs) {
            ...
            while (my ($prob, $valp) = each %probs) {
				...
				my $fac_prob_id = lookup_facility_prob($fac_id, $component_class, $component);
				#if ($fac_prob_id <= 0) {
					# error looking up ID
				#	$err_cnt++;
				#	next;
				...
				$sth_ins_prob->execute($fac_id, $component_class, $component, $prob,
					...
					$alpha, $beta, 'admin', SC::time_to_ts(time), $metric);
