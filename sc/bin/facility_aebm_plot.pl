...
foreach my $facility (keys %{$json_text}) {
    ...
    $spect_data .= "e\n";
    $spect_data = join '', ($spect_data, $spect_data, $spect_data);

    my $perm_point = join '', (
                $permformance->{'median'}->{'sd'}, ' ', $permformance->{'median'}->{'sa'},
                "\ne\n");
    my $perm_obj_cmd = join '', (
	...
	$cmd = 'plot "-" using 1:2:3:4:5:xticlabels(6) with boxerrorbars lc variable, "-" using 1:2:2 with labels';
	$ds_data .= "e\n";
  $ds_data .= $ds_data;
  ...
  foreach my $ind (0 .. scalar @{$capacity->{'sa'}} - 1) {
	...
	$cmd = 'plot "-" using 6:7 title "Capacity Lower/Upper Bound" with lines lt 53, "-" using 8:9 notitle with lines lt 53, "-" using 10:11 title "Demand Lower/Upper Bound" with lines lt 54, "-" using 12:13 notitle with lines lt 54, "-" using 2:3 title "Capacity" with linespoints lt 51, "-" using 4:5 title "Demand" with linespoints lt 52, "-" title "Performance Point" with points pt 3 lw 3 lc "green"';
	$perm_data .= "e\n";
  $perm_data = join '', ($perm_data, $perm_data, $perm_data, $perm_data, $perm_data, $perm_data, $perm_point);
	$output_png = "$dir_path/$facility"."_perm.png";
	send_data_to_perm_plot($event, $facility, $output_png, $label, $max_value, $perm_obj_cmd, $cmd, $perm_data);

	$cmd = 'plot "-" using 2:4 title "Raw 5%-Damped 3-Domain Spectrum" with lines lt 53, "-" using 2:3 title "Smoothed Spectrum" with linespoints lt 51, "-" using 5:6 title "Demand Spectrum" with linespoints lt 52';
...
set linetype 53 lw 2 lc rgb "dark-cyan" 
...
set linetype 52 lw 2 lc rgb "red"
set linetype 53 lc rgb "light-blue" 
$obj_cmd
