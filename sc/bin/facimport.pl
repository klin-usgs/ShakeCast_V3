...
sub process {
    ...
    while (!$fh->eof) {
        ...
        my $ext_id = $colp->[$columns{EXTERNAL_FACILITY_ID}];
        my $type   = $colp->[$columns{FACILITY_TYPE}];
        ...
        if (%metrics) {
            while (my ($metric, $valp) = each %metrics) {
                if ($mode == M_UPDATE) {
                    ...
                    # deleted
                    $sth_del_one_metric->execute($fac_id, $metric);
                }
		my $level;
		my $lo;
		foreach my $ix (0 .. 3) {
...
sub usage {
    ...
    print qq{
facimport -- Facility Import utility
Usage:
  facimport [ mode ] [ option ... ] input-file
...
facimport - ShakeCast Facility import tool

=head1 DESCRIPTION

The B<facimport> utility is used to insert or update facility data in ShakeCast.
...
B<facimport> reads from one or more CSV-formatted files.
...
These headers tell facimport how to interpret the rest of the records.
...
=head1 Invocation
  ...
  facimport [ mode ] [ option ... ] file.csv [ file2.csv ... ]
...
C<facimport> will operate in C<replace> mode if you do not specify a mode.
...
the command
  ...
  facimport ca_cities.csv

=head2 Example 2 -- Fragility parameters

It is easy to load fragility parameters for your facilities using B<facimport>.
...
This file would be loaded using the command
  ...
  facimport --update city_county.csv
