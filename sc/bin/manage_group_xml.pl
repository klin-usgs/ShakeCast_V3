...
sub creae_notification_profile {
	...
	my $result = $sth_ins_notification_request->execute(
       ...
       (ref $colp->{MESSAGE_FORMAT} ne 'HASH') ? $colp->{MESSAGE_FORMAT} : '',
       (ref $colp->{LIMIT_VALUE} ne 'HASH') ? $colp->{LIMIT_VALUE} : undef,
       $colp->{USER_MESSAGE},
       $colp->{NOTIFICATION_PRIORITY},
       $colp->{AUXILIARY_SCRIPT},
       $colp->{DISABLED},
       (ref $colp->{PRODUCT_TYPE} ne 'HASH') ? $colp->{PRODUCT_TYPE} : '',
       (ref $colp->{METRIC} ne 'HASH') ? $colp->{METRIC} : '',
       $colp->{AGGREGATE},
