...
sub process {
    my @servers = SC::Server->servers_to_poll;
    if (@servers) {
        foreach my $server (@servers) {
            $server->poll_for_updates or SC->error($server->server_id, $SC::errstr);
            GenericDaemon::spoll;
        }
    } elsif ($SC::errstr) {
        SC->error($SC::errstr);
