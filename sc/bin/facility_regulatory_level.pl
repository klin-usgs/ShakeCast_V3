...
foreach my $facility (@$fac_list) {
	...
	my %reg_rv;
	$reg_rv{felt} = felt($fac_id);
	$reg_rv{felt_site} = felt_site($fac_id);
	$reg_rv{mag_dist} = mag_dist($fac_id);
	$reg_rv{rg1166} = ($reg_rv{felt} || $reg_rv{felt_site} || $reg_rv{mag_dist})
...
$xs->toFile("$dir_path/facility_regulatory_level.xml");
