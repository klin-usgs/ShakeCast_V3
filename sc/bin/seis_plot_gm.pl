...
#http://maps.google.com/maps/api/staticmap?center=40,-70&=6&size=800x800&markers=icon:http://earthquake.usgs.gov/research/software/shakecast/icons/epicenter.png|40,-70&sensor=false&&path=fillcolor:0xAA000033|color:0xFFFFFF00|45,-75|45,-65|35,-65|35,-75|45,-75&&path=fillcolor:0xAA000033|color:0xFFFFFF00|42,-72|42,-68|38,-68|38,-72|42,-72
...
my $epi_lon;
if ($earthquake) {
  $epi_lat = sprintf("%.2f", $earthquake->{'lat'});
  $epi_lon = sprintf("%.2f", $earthquake->{'lon'});
...
#foreach my $facility (@{$facility_data}) {
  ...
  $center = "$epi_lat,$epi_lon";
  #$icon = lc($fac_type).".png";
  #print "$fac_type, $fac_id, $fac_lat, $fac_lon\n";
  #$output_png = "$sc_dir/$event/epicenter_".(lc($fac_type))."_$fac_id.png";
  $output_png = "$sc_dir/$event/epicenter.png";
  ($cir3, $min_lon, $max_lon, $min_lat, $max_lat) = circle_dist($epi_lon, $epi_lat, 150);
  ($cir10, $min_lon, $max_lon, $min_lat, $max_lat) = circle_dist($epi_lon, $epi_lat, 500);
  foreach  my $event (@{$earthquake_catalog}) {
    ...
    next unless ($lat_h > $min_lat-3 && $lat_h < $max_lat+3 && 
      ($lon_h > $min_lon-3 || $lon_h < ($min_lon+360+3)) &&
      ($lon_h < $max_lon+3 || $lon_h > ($max_lon-360-3)) );
    #next if (dist($fac_lat, $fac_lon, $lat_h, $lon_h) > 500);
    next if (dist($epi_lat, $epi_lon, $lat_h, $lon_h) > 900);
  ...
  #$url = "http://maps.google.com/maps/api/staticmap?center=$center&zoom=6&size=800x800$epicenter&markers=icon:http://shakecast.awardspace.com/images/$icon|$center&path=fillcolor:0xAA000033|color:0xFFFFFF00|$cir10&path=fillcolor:0xAA000033|color:0xFFFFFF00|$cir3&sensor=false";
  $url = "http://maps.google.com/maps/api/staticmap?center=$center&zoom=6&size=800x800$epicenter&path=fillcolor:0xAA000033|color:0xFFFFFF00|$cir10&path=fillcolor:0xAA000033|color:0xFFFFFF00|$cir3&sensor=false";
