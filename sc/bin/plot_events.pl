...
GetOptions(
    ...
    'mag=f',           # error for existing facilities
    'gmpe=s',           # error for existing facilities
    'help',             # print help and exit
    'verbose',             # print help and exit

    'dev=f',          # max bad records allowed (0 for no limit)
    
) or usage(1);
#usage(1) unless ($options{'mag'});

my $mag = $options{'mag'};
my $dev = (defined $options{'dev'}) ? $options{'dev'} : 0.1;
my $verbose = $options{'verbose'} if $options{'verbose'};
my $gmpe = $options{'gmpe'} if $options{'gmpe'};
...
$sth_lookup_mag = SC->dbh->prepare(qq{
	select sm.shakemap_id, max(sm.shakemap_version) as shakemap_version, g.grid_id 
		from (shakemap sm 
			inner join grid g on sm.shakemap_id = g.shakemap_id 
			and sm.shakemap_version = g.shakemap_version), event e 
		where sm.shakemap_id = e.event_id 
			and e.magnitude > ? 
			and e.magnitude < ? 
		group by e.event_id});
...
$sth_facility_metric = $dbh->prepare(qq{
    ...
    vpr "Processing Magnitude: $mag, Span: $dev";
    process();
...
sub process {
	...
	my %types = ('PGA'=>1, 'PGV'=>1, 'PSA03'=>1, 'PSA10'=>1, 'PSA30'=>1);
	my %info;
	if (defined $mag) {
		%info = ('MAG'=>$mag, 'DEV'=>$dev);
		$info{'GMPE'} = $gmpe if (defined $gmpe);
	} else {
		%info = ('MAG'=>'Varied', 'DEV'=>'NA');
	}
	
	if (defined $mag && defined $dev) {
		$sth_lookup_mag->execute($mag-$dev, $mag+$dev);
		while (my $event = $sth_lookup_mag->fetchrow_hashref('NAME_uc')) {
			my $facility_shaking = facility_station_shaking($event);
			#print $event->{SHAKEMAP_ID},'-',$event->{SHAKEMAP_VERSION},':',
				$event->{GRID_ID},":";
			#print scalar @$facility_shaking if (defined $facility_shaking);
			#print "\n";
			my $shakemap_info = lookup_shakemap_info(
				$event->{SHAKEMAP_ID},$event->{SHAKEMAP_VERSION});
			next if (defined $gmpe && $shakemap_info->{GMPE} !~ /\:\:$gmpe$/);
			push @facility_shakings, @$facility_shaking 
				if (defined $facility_shaking);
		}
	} elsif (@ARGV) {
		foreach my $shakemap (@ARGV) { 
			my ($shakemap_id, $shakemap_version) = split '-', $shakemap;
			my %event = ('SHAKEMAP_ID'=>$shakemap_id, 'SHAKEMAP_VERSION'=>$shakemap_version,
				'GRID_ID' => lookup_grid($shakemap_id, $shakemap_version));
			my $facility_shaking = facility_station_shaking(\%event);
			#print $event{SHAKEMAP_ID},'-',$event{SHAKEMAP_VERSION},':',
				$event{GRID_ID},":";
			#print scalar @$facility_shaking if (defined $facility_shaking);
	...
	print "\n ",scalar @facility_shakings,"\n";
	make_plot(\%types, \@facility_shakings,\%info);
}


sub facility_station_shaking {
    my $event = shift;
    my $err_cnt = 0;
    my $nrec = 0;
    my $nins = 0;
	...
	my @facility_shaking;
	$sth_grid->execute($event->{'GRID_ID'});
	my $metric_query;
	...
	my $sql = "SELECT f.facility_id,f.external_facility_id,f.lat_min, 
		...
		WHERE 
			g.grid_id = \"".$event->{'GRID_ID'}."\"";
	my $facility_shaking_query = $dbh->prepare($sql);
	$facility_shaking_query->execute();
	while (my $facility_shaking = $facility_shaking_query->fetchrow_hashref('NAME_uc')) { 
		if (defined $facility_shaking) {
	...
	#foreach my $fac (@facility_shaking) {
	#	print "fac: $fac->{FACILITY_ID}\n";
	#}
...
#    my $src = shift;
	...
	my $prefix = $info->{MAG}.'_'.$info->{DEV}.'_';
	$prefix .= $info->{GMPE}.'_' if (defined $info->{GMPE});
	my $outputdir = SC->config->{'DataRoot'} . "/_PLOT/";
    foreach my $type (keys %$types) {
		my $plotfile = "$outputdir/$prefix$type.ps";
		my $pngfile = "$outputdir/$prefix$type.png";
		unlink $plotfile;
		unlink $pngfile;
	}
	...
	my    $min_dist = 10;   # Minimum distance to plot in km
	my    $max_dist = 200.0;
	my    $min_y = 1;    # Min Y scale, acceleration %g or velocity cm/sec
	my    $max_y = 10.0;
	my (@rows, $command);
	#----------------------------------------------------------------------
    ...
    foreach my $type (keys %$types) {
		...
		my $gmt_acc_B = "-Ba1g1f3:\"Distance\":/a1g1f3:\"\% g\"::.$prefix$type:WeSnZ";
		my $gmt_vel_B = "-Ba1g1f3:\"Distance\":/a1g1f3:\"cm/sec\"::.$prefix$type:WeSnZ";

		# Compute the plot boundaries min/max
		my (@dist, @yval, $x, %y, $sta);
		$x = $min_dist;
		foreach $sta (@$staref) {
			next unless ($sta->{DISTANCE});
			$max_dist = $sta->{DISTANCE} if ($max_dist < $sta->{DISTANCE});
			$min_dist = $sta->{DISTANCE} if ($min_dist > $sta->{DISTANCE});
			next unless (defined $sta->{$type});
			$max_y = $sta->{$type} if ($max_y < $sta->{$type});
			$min_y = $sta->{$type} if ($min_y > $sta->{$type});
		}
		$max_dist *= 2;
		$min_dist /= 2;
		$max_y *= 2;
		...
		my $rlogflag = "-R$min_dist/$max_dist/$min_y/$max_y";
		...
		$command = "| $gmt_bin/psxy $rlogflag $jlogflag " .
			"-K -O -Wthin,- >> $plotfile";
		print "Running $command" if ($verbose);
		foreach $sta (@$staref) {
			next unless ($sta->{$type} && $sta->{$sta_type});
			open PLOT, $command or die "Can't run: $command";
				printf PLOT "%f %f\n", $sta->{DISTANCE}, $sta->{$type};
				printf PLOT "%f %f\n", $sta->{DISTANCE}, $sta->{$sta_type};
			close PLOT;
		}
		...
		foreach $sta (@$staref) {
			...
			my $sym_color = ($sta->{$type} > abs($sta->{$sta_type})) ? '255/0/0' : '0/255/0';
