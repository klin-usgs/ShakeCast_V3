...
$sth_ins_user = SC->dbh->prepare(qq{
    insert into shakecast_user (
...
$sth_upd_user = SC->dbh->prepare(qq{
    update shakecast_user
		set email_address = ?,
		phone_number = ?, 
		full_name = ?,
		password = ?,
...
sub process {
	...
	foreach my $colp (@data) {
        ...
        my $ext_id = $colp->{USERNAME};
        my $type   = $colp->{USER_TYPE};
        my $password   = $colp->{PASSWORD}
			unless (ref  $colp->{PASSWORD} eq 'HASH');
        my $full_name   = $colp->{FULL_NAME}
			unless (ref  $colp->{FULL_NAME} eq 'HASH');
        my $email_address   = $colp->{EMAIL_ADDRESS}
			unless (ref  $colp->{EMAIL_ADDRESS} eq 'HASH');
        my $phone_number   = $colp->{PHONE_NUMBER}
			unless (ref  $colp->{PHONE_NUMBER} eq 'HASH');
        my $delivery   = $colp->{DELIVERY};
        my $group   = $colp->{GROUP}
			unless (ref  $colp->{GROUP} eq 'HASH');
        
        my $shakecast_user = lookup_user($ext_id);
	    #my $csh = Crypt::SaltedHash->new(algorithm => 'SHA-256');
	    print "$password\n";
	    if (defined $password) {
		system ("$perl $update_password -target user -username $ext_id -password ".$password);
        ...
        if ($shakecast_user < 0) {
            ...
            eval {
                $sth_ins_user->execute($email_address, $phone_number,
                	$full_name, $password, $ext_id, $type, 
                ...
                if ($shakecast_user < 0) {
					...
					my $password = $colp->{PASSWORD};
					my $user_level = ($type eq 'ADMIN') ? 1 : 0;
            ...
            if ($mode == M_SKIP) {
                ...
                eval {
                    ...
                    $sth_upd_user->execute($email_address, $phone_number,
                	$full_name, $password, $ext_id, $type, 
					'admin', SC::time_to_ts(time), $shakecast_user);
                    $nrepl++;
					$shakecast_user = lookup_user($ext_id);
					if ($shakecast_user < 0) {
						$err_cnt++;
						...
						my $password = $colp->{PASSWORD};
						my $user_level = ($colp->{USER_TYPE} eq 'ADMIN') ? 1 : 0;
						$sth_ins_lookup_user->execute($password, $user_level, $shakecast_user);
					}
                };
                if ($@) {
        ...
        if ($delivery) {
            ...
            while (my ($method, $address) = each %{$delivery}) {
                # don't insert null attribute values
                next unless defined $address and $address ne '';
                $sth_ins_method->execute($shakecast_user, $method, $address);
            }
        }
