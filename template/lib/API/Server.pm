...
sub BEGIN {
    no strict 'refs';
    for my $method (qw(
			server_id dns_address ip_address owner_organization
			last_heard_from error_count system_generation software_version
			birth_timestamp death_timestamp pki_key server_status
			lat lon last_event_timestamp number_of_users access_count
			event_count product_count update_username update_timestamp
			password upstream_flag downstream_flag poll_flag query_flag
			self_flag event_hwm shakemap_hwm product_hwm
			receive_timestamp 
			)) {
	*$method = sub {
	    my $self = shift;
...
sub from_id {
	...
	$SC::errstr = $@;
	return undef;
    } elsif (not defined $server) {
	$SC::errstr = "No Server for id $server_id";
    }
    return $server;
}

sub update_info {
    my ($class, $server_id, $dns_address, $query_flag) = @_;

	$query_flag = ($query_flag =~ /true/i) ? 1 : 0;
    undef $SC::errstr;
    my $server;
    my $sth;
	my $result;
	...
	$sth = SC->dbh->prepare(qq/
	    update server
		SET 
			server_id = ?, dns_address = ?,
			query_flag = ?
		where server_id = ?
	    /);
	$result = $sth->execute($server_id, $dns_address, $query_flag, $server_id);
	$sth->finish;
...
sub server_list {
	...
	my $sql = qq/
		...
		FROM 
			server
		WHERE
			server_id NOT IN (1, 1000)
		/;
    eval {
	my $sth = SC->dbh->prepare($sql);
	$sth->execute();
	while (my $p = $sth->fetchrow_arrayref) {
		my %server = %{$class->from_id(@$p)};
	    push @servers, \%server;
	}
	$sth->finish;
    };
...
sub Server_type_list {
	...
	my $sql = qq/
		...
		FROM 
			Server
		/;
	$sql .= " where Server_network ='$type'" unless ($type=~ /all/i);
...
sub process_new_event {
	...
	return 1;
		...
		if ($self->event_status ne 'CANCELLED') {
			...
			#if ($self->process_Server_model) {
			#	SC->log(2, "Server model processed");
			#} else {
			#	SC->error($SC::errstr);
