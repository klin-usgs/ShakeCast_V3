...
sub BEGIN {
    ...
    for my $method (qw(
			...
			url display product_source count
			event_location_description event_timestamp
			superceded_timestamp 
			)) {
	*$method = sub {
	    my $self = shift;
    ...
    my $sql =  qq/
        select s.shakemap_id,
         ...
         where s.seq > ?
		   ...
		   and datediff(now(), e.event_timestamp) < ?
		ORDER BY
				e.seq DESC
		/;
	$oldest = (SC->config->{rss}->{TIME_WINDOW}) unless ($oldest);
	push @args, $oldest;
...
sub from_id {
    my ($self, $shakemap_id, $shakemap_version) = @_;
	...
	$sth = SC->dbh->prepare(qq/
	    select shakemap_id, shakemap_version, generation_timestamp, receive_timestamp, 
			lat_min, lat_max, lon_min, lon_max, product_status, generating_server, update_timestamp
	      from product 
	     where shakemap_id = ?
	...
	$shakemap = new API::Product(%$p);
	$sth->finish;
 	my @products;
	$sth = SC->dbh->prepare(qq/
	    select p.product_type, p.product_id, p.max_value, p.min_value, 
			p.product_file_exists, pt.name, pt.description, pt.metric, pt.filename, pt.url,
			pt.product_source, pt.display
	      from product_type pt 
			left join product p on (p.product_type = pt.product_type)
			and (p.shakemap_id = ? and p.shakemap_version = ?)
		 /);
	$sth->execute($shakemap_id, $shakemap_version);
	while ($p = $sth->fetchrow_hashref('NAME_lc')) {
	    my $filepath;
	    if ((lc($p->{product_source}) ne 'shakemap') && (lc($p->{product_source}) ne 'shakecast')) {
		if (lc($p->{product_source}) eq 'dyfi') {
		    $filepath = SC->config->{'DataRoot'}.'/eq_product/'.$shakemap_id.'/'.$shakemap_id.'_'.$p->{filename};
		    next unless (-e $filepath);
		    $p->{filename} = 'eq_product/'.$shakemap_id.'/'.$shakemap_id.'_'.$p->{filename};
		} else {
		    $filepath = SC->config->{'DataRoot'}.'/eq_product/'.$shakemap_id.'/'.$p->{filename};
		    next unless (-e $filepath);
		    $p->{filename} = 'eq_product/'.$shakemap_id.'/'.$p->{filename};
		}
		$p->{product_file_exists} = 1;
		$p->{eq_product} = 1;
	    } else {
		next unless ( -e SC->config->{'DataRoot'}.'/'.$shakemap_id.'-'.$shakemap_version.'/'.$p->{filename}
			     && $p->{display});
	    }
...
sub product_source_list {
    ...
    my @products;
	my $sql = qq/
		SELECT product_source, count(product_type) as count
		FROM 
			product_type
		GROUP BY
			product_source
	...
	while (my $p = $sth->fetchrow_hashref('NAME_lc')) {
	    push @products, new API::Product(%$p);
	}
	$sth->finish;
    };
    if ($@) {
...
sub product_type_list {
	...
	my $sql = qq/
		...
		FROM 
			product_type
		/;
	$sql .= " where product_source ='$type'" unless ($type=~ /all/i);
...
sub toggle_product_display {
	...
	my ($nrec) = SC->dbh->selectrow_array(qq/
        ...
        unless ($nrec) {
            $SC::errstr = "Can't update product type $product_id";
            return 0;
        }
...
sub current_shakemap {
	...
	$sth = SC->dbh->prepare(qq/
	    select shakemap_id,
                   shakemap_version
	      from shakemap
	     where (superceded_timestamp IS NULL)
		ORDER BY
			seq DESC
		 /);
	$sth->execute();
...
sub is_local_test {
    my $self = shift;
    my $event = API::Event->from_id($self->event_id, $self->event_version);
...
sub eq_product_dir {
    my $self = shift;
    return SC->config->{'DataRoot'} . '/eq_product/' .
	    $self->{'shakemap_id'};
}
